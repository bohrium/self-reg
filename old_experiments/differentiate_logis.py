''' author: samtenka
    ...
    descrp: Estimate and save gradient statistics (on binary MNIST)
            To run, type:
                python differentiate_logis.py 10000 24 32 gradstats_logis.txt
            ...
            the    gradstats_logis.txt   gives   a filename to overwrite with results.
'''
...
class Learner(object):
    ''' Creates, (re)initializes, trains, and evaluates a logis neural network. '''
    ...
    def create_model(self, batch_size, precision=tf.float32):
        ...
        self.WeightsA = tf.transpose(tf.reshape(self.Weights[:sizeA,:], [28, 28, batch_size]), perm=(2,0,1))
        self.WeightsB = tf.transpose(tf.reshape(self.Weights[sizeA:sizeA+sizeB,:], [28, 28, batch_size]), perm=(2,0,1))
        self.WeightsC = tf.transpose(tf.reshape(self.Weights[sizeA+sizeB:,:], [28, 1, batch_size]), perm=(2,0,1))
        ...
        self.Logits = tf.reshape(tf.matmul(tf.expand_dims(self.Images, 1), self.WeightsC), [-1])
    ...
    def sample_init_weights(self): 
        ...
        wb = 1.0*(np.arange(0.0,28.0*28.0)/(28.0*25.0)-0.5) #/(25.0+25.0)**0.5

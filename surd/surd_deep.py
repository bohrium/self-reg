...
class Dataset(object): 
    ...
    def get_batch(self, batch_size, split='ins', opt='sgd', with_replacement=False):
        ...
        if self.index == self.sample_size: # shuffle 
            assert(self.index == self.sample_size)
            indices = np.random.shuffle(np.arange(self.sample_size))
            self.sample_images = self.sample_images[indices][0] 
            self.sample_labels = self.sample_labels[indices][0]
            self.index = 0
        rtrn = (self.sample_images[self.index : self.index+batch_size],
                self.sample_labels[self.index : self.index+batch_size])
...
class Learner(object):
    ...
    def create_trainer(self, precision=tf.float32):
        ...
        self.SURDGradientWeights = tf.convert_to_tensor(tf.gradients(self.SURDLoss, self.Weights))[0]
        self.SURDUpdate = tf.tuple([
            tf.assign(self.Weights, self.Weights - self.LearningRate * self.SURDGradientWeights), 
        ])
    ...
    def run(self, dataset, ins_time, batch_size, learning_rate, opt='sgd', beta=BETA): 
        ...
        for t in range(ins_time):
            ...
            if opt=='surd':
                self.session.run([self.SURDUpdate], feed_dict={

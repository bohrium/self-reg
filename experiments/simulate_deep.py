''' author: samtenka
    ...
    descrp: Append summary of SGD and GD losses (on logistic regression on MNIST 0-vs-1 classification) to a file.
            ...
            To run, type:
                python simulate_deep.py 1000 100 0.00 0.005 12 32 experdata_deep.txt 
...
class Learner(object):
    ...
    def create_model(self, precision=tf.float32):
        ...
        self.WeightsA = tf.reshape(self.Weights[:sizeA], [28*28, 85])
        self.WeightsB = tf.reshape(self.Weights[sizeA:sizeA+sizeB], [85, 9])
        self.WeightsC = tf.reshape(self.Weights[sizeA+sizeB:], [9, 1])

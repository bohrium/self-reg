...
class Learner(object):
    ...
    def create_model(self, batch_size, precision=tf.float32):
        ...
        self.Weights = tf.get_variable('flattened', shape=[28*28*1, batch_size], dtype=precision)
        self.WeightsA = tf.transpose(tf.reshape(self.Weights, [28*28, 1, batch_size]), perm=(2,0,1))
        ...
        self.Logits = tf.reshape(tf.matmul(tf.expand_dims(self.Images, 1), self.WeightsA), [-1])

''' author: samtenka
    ...
    descrp: Estimate and save gradient statistics (on binary MNIST)
            To run, type:
                python differentiate_shallow.py 10000 24 32 gradstats_shallow.txt
            ...
            the   gradstats_shallow.txt   gives   a filename to overwrite with results.
'''
...
class Learner(object):
    ...
    def create_model(self, batch_size, precision=tf.float32):
        ...
        sizeA = 28*28*28
        sizeB = 28*1
        self.Weights = tf.get_variable('flattened', shape=[sizeA + sizeB, batch_size], dtype=precision)
        self.WeightsA = tf.transpose(tf.reshape(self.Weights[:sizeA,:], [28*28, 28, batch_size]), perm=(2,0,1))
        self.WeightsB = tf.transpose(tf.reshape(self.Weights[sizeA:,:], [28, 1, batch_size]), perm=(2,0,1))
